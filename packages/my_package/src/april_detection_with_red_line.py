...
class DShapeNode(DTROS):
    ...
    def lane_follow(self):
        ...
        while not rospy.is_shutdown():
            ...
            if yellow_msg is not None and white_msg is not None:
                ...
                cmd = Twist2DStamped(v=self.VELOCITY/2 , omega=self.OMEGA_SPEED*1.5)
                self.pub_cmd.publish(cmd)
    ...
    def turn(self, angle=90):
        ...
        rate = rospy.Rate(100)
        start_angle = self._right_distance_traveled
        while not rospy.is_shutdown():
            angle_turned = (self._right_distance_traveled - start_angle) / self.BASELINE
            if angle_turned >= angle_rad:
                self.stop()
                rospy.loginfo("Stopping: Angle limit hit.")
                break
            cmd = Twist2DStamped(v=self.VELOCITY, omega=self.OMEGA_SPEED)
            self.pub_cmd.publish(cmd)
            rate
            rate.sleep()

    def straight(self, distance=0.5):
        rospy.loginfo("Starting straight line until 10 meters...")
        self.set_led_color('GREEN')
        #making the bot go straight using wheel encoders
        rate = rospy.Rate(100)
        while not rospy.is_shutdown():
            avg_distance = (self._left_distance_traveled + self._right_distance_traveled) / 2
            if avg_distance >= distance:
                self.stop()
                rospy.loginfo("Stopping: Distance limit hit.")
                break
